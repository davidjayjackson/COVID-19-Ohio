...
### State Data
rm(list=ls())
db <- dbConnect(RSQLite::SQLite(),dbname= "../COVID-19-DB/NYTimes.sqlite3")
df <- dbGetQuery(db,"select * from COUNTYDAILY WHERE state ='Ohio'")
```
```{r}
df$date <- as.Date(df$date)
df <- df[order(df$date),]
df <- subset(df, date >="2020-03-10" )
...
#### Calc and Plot Daily Mean/Median Cases and Deaths for Ohio
(Based on daily county data for Ohio)

```{r}
buckeye <- df %>% group_by(date) %>%
...
```
```{r}
ggplot(buckeye) + geom_line(aes(x=date,y=median_cases)) +
  scale_y_continuous(labels = comma) + 
  labs(title = "Ohio Median Cases by Date")

ggplot(buckeye) + geom_line(aes(x=date,y=mean_cases)) +
  scale_y_continuous(labels = comma) + 
  labs(title = "Ohio Mean Cases by Date")
  
```
```{r}
ggplot(buckeye) + geom_line(aes(x=date,y=median_deaths)) +
  scale_y_continuous(labels = comma) + 
  labs(title = "Ohio Median Deaths by Date") + ylim(0,4)

ggplot(buckeye) + geom_line(aes(x=date,y=mean_deaths)) +
  scale_y_continuous(labels = comma) + 
  labs(title = "Ohio Mean Deaths by Date") + ylim(0,10)
...
```{r}
df$CPC <- (df$new_cases/116900000)*100000
ggplot(df) + geom_line(aes(x=date,y=CPC)) +
  scale_y_continuous(labels= comma) +
  labs(title="Ohio Case Per 100,000")
```
```{r}
df$DPC <- (df$new_deaths/116900000) *100000
ggplot(df) + geom_line(aes(x=date,y=DPC)) +
  scale_y_continuous(labels = comma) +
  labs(title="Ohio Deaths Per 100,000") + theme_light()
