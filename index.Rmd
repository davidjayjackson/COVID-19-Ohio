...
```{r}
# US$new_deaths <- ifelse(US$date =="2021-02-11",mean(US$new_deaths),US$new_deaths)
# US$new_deaths <- ifelse(US$date =="2021-02-12",mean(US$new_deaths),US$new_deaths)
# US$new_deaths <- ifelse(US$date =="2021-02-13",mean(US$new_deaths),US$new_deaths)
US <- df %>% filter(date >="2020-04-01")

US$MAC <- movavg(US$new_cases,60,type="e")
US$MAD <- movavg(US$new_deaths,60,type="e")

ggplot(US,aes(x=date,y=MAC)) + geom_col() +
    labs(title="Ohio Daily COVID19 Cases by Date",
         subtitle = "(60 Day Moving Average)") 
  

ggplot(US,aes(x=date,y=MAD)) + geom_col() +
    labs(title="Ohio Daily COVID19 Deaths by Date",
         subtitle = "(60 Day Moving Average/ y-limit:0,100)") +
  ylim(0,100)
```
