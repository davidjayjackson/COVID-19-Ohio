...
```{r,echo=FALSE}
STATESDAILY %>% plot_ly() %>% add_bars(x=~date,y=~new_cases) %>%
  layout(title="US Total Daily Cases by Date")

STATESDAILY %>% plot_ly() %>% add_bars(x=~date,y=~new_deaths) %>%
  layout(title="US Total Deaths by Date")

STATESDAILY %>% plot_ly() %>% add_bars(x=~date,y=~death_rate) %>%
