...
```
```{r}
df$new_deaths <-  ifelse(df$date >="2020-10-01" & df$date <="2020-12-31",df$new_deaths + 30,df$new_deaths)
```
```{r}

df$new_deaths <-  ifelse(df$date =="2021-02-12",800,df$new_deaths)
df$new_deaths <-  ifelse(df$date =="2021-02-13",800,df$new_deaths)
...
Trend <- US
US$MAC <- ma(US$new_cases,28,centre=TRUE)
US$MAD <- ma(US$new_deaths,28,centre=TRUE)

ggplot(US,aes(x=date,y=MAC)) + geom_line(lwd=1,col="red") +
    labs(title="Ohio Daily COVID19 Cases by Date",
         subtitle = "(28 Day Moving Average)") 
  

ggplot(US,aes(x=date,y=MAD)) + geom_line(lwd=1,col="red") +
    labs(title="Ohio Daily COVID19 Deaths by Date",
         subtitle = "(28 Day Moving Average/ y-limit:0,100)") +
  ylim(0,100)
```
...
# df <- subset(DF,date ==Sys.Date() -8 )
# Top <- df[order(df$new_deaths,decreasing = TRUE),]
# Top5 <-top_n(df,5,new_deaths)
