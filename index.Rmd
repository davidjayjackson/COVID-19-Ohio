...
### Top Five Counties Based on Daily Totals for Sept. 5, 2020

```{r}
df <- subset(DF,date =="2020-09-05" )
Top <- df[order(df$new_cases,decreasing = TRUE),]
Top5 <-head(Top,n=5)
barplot(new_cases ~county,data=Top5,main="Top 5 Counties Ranked by single Day Cases",xlab="(For Sept 5, 2020)",ylab="Cases")
grid(col="black")

Topd <- df[order(df$new_deaths,decreasing = TRUE),]
Top5d <-head(Topd,n=5)
barplot(new_deaths ~county,data=Top5d,main="Top 5 Counties Ranked by single Day Deaths",xlab="(For Sept 5, 2020)",ylab="Cases")
grid(col="black")
```
