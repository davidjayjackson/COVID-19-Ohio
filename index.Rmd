...
buckeye %>% ggplot() +
  geom_col(aes(x=date,y=Cases)) + 
     ...
     scale_y_log10(labels = scales::comma) + geom_smooth(aes(x=date,y=Cases),method="glm")
```
```{r}
buckeye %>% ggplot() +
  geom_col(aes(x=date,y=Deaths)) + 
     labs(title="Ohio Accumulated Deaths by Date",x="Date",y="Accumulated Cases(log10)") +
     scale_y_log10(labels = scales::comma) + geom_smooth(aes(x=date,y=Deaths),method="glm")
```
