...
```{r,echo=FALSE}
gg1 <- STATESDAILY %>% filter(new_cases >0) %>% ggplot() + 
  geom_col(aes(x=date,y=new_cases)) +
      labs(title="Ohio Cases by Date") +
     theme(axis.text.x = element_text(angle = 45)) 
    
gg2 <- STATESDAILY %>% filter(new_deaths >0)%>% ggplot() + geom_col(aes(x=date,y=new_deaths)) +
      labs(title="OhioDeaths by Date") +
   theme(axis.text.x = element_text(angle = 45))

gg3 <- STATESDAILY %>% filter(new_cases >0) %>% ggplot() + geom_col(aes(x=date,y=death_rate)) +
