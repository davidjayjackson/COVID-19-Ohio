...
ggplot(buckeye) + geom_line(aes(x=date,y=new_cases),lwd=1,col="red") +
  ...
  geom_smooth(aes(x=date,y=new_cases),method="gam")
...
buckeye <- df %>% filter(date >="2020-10-01") 
ggplot(buckeye) + geom_line(aes(x=date,y=new_cases),lwd=1,col="red") +
  labs(title="Ohio Cases (GAM)",y="21 Day Moivng Average",
       x="Dates Starting with Dec. 1,2020") +
...
```{r}
buckeye <- df %>% filter(date >="2020-10-01") 
ggplot(buckeye) + geom_line(aes(x=date,y=new_deaths),lwd=1,col="red") +
...
buckeye <- df %>% filter(date >="2020-10-01") 
ggplot(buckeye) + geom_line(aes(x=date,y=new_deaths),lwd=1,col="red") +
  labs(title="Ohio Deaths (GAM)",y="21 Day Moivng Average",
       x="Dates Starting with Dec. 1,2020") +
  geom_smooth(aes(x=date,y=new_deaths),method = "gam") + ylim(0,225)
